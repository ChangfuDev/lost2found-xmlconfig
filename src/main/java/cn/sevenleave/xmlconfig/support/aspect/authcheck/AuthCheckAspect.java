...
public class AuthCheckAspect {
    ...
    //-- 通过切点来选择连接点，详见手册 --
    // 1.限定匹配带有指定注解的连接点
    @Pointcut("@annotation(cn.sevenleave.xmlconfig.support.aspect.authcheck.AuthCheck)")
    ...
    public Object checkAuth(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if (!userToken.equals(AuthCheckEnum.MY_TEMPORARY_USERTOKEN.getUserToken())) {
            // 日志输出
    ...
    private String getUserToken(HttpServletRequest request) {
        ...
        for (Cookie cookie : cookies) {
            if ("userToken".equals(cookie.getName())) {
                // 测试使用的token值
                return AuthCheckEnum.MY_TEMPORARY_USERTOKEN.getUserToken();
            }
        }
        return "";
    }
